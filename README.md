...
本项目使用的核心语料库是两个高质量推理蒸馏数据集：

- `nohurry/Opus-4.6-Reasoning-3000x-filtered`：提供 `problem / thinking / solution` 结构的数据，适合学习复杂问题的分解、推理过程和最终答案组织方式。
- `Jackrong/Qwen3.5-reasoning-700x`：由 Qwen3.5-27B 教师模型蒸馏生成，覆盖数学、代码、逻辑、科学和通用指令等场景，天然包含 `<think>...</think>` 推理格式。

选择这些语料库的原因，是它们不是普通闲聊数据，而是带有明确推理轨迹的高质量监督信号。训练脚本会把不同来源的数据统一转换成 Qwen thinking 对话格式，使小模型不仅学习答案本身，也学习“先推理、再作答”的行为模式。

这些语料库微调主要解决三个问题：

- **提升小模型推理能力**：原始 2B 模型部署成本低，但在数学、代码、逻辑和复杂指令场景中容易跳步、短答或推理不稳定。推理蒸馏语料能为模型提供更清晰的解题路径监督。
- **稳定模型输出格式**：项目强制 assistant 回复符合 `<think>...</think>\n最终答案` 格式，便于后续推理展示、答案解析、部署接入和行为控制。
- **降低强模型使用成本**：微调的核心意义，是把这些语料背后教师模型体现出的推理风格、领域能力和回答优势迁移到更小的 Qwen3.5-2B 模型中，在尽量保留能力收益的同时降低显存占用、推理成本和部署门槛。

因此，本项目并不是简单地“让小模型多看一些数据”，而是通过 reasoning SFT 将强模型蒸馏数据中的能力压缩进小模型，让本地、低成本、可控的模型也具备更好的复杂问题处理能力。
